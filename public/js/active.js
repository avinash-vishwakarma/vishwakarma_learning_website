...
let passWord = document.querySelectorAll(".showPassword");

if (passWord.length > 0) {
    passWord.forEach((passwordField) => {
        passwordField.addEventListener("click", passwordFunction);
    });
}

function passwordFunction(e) {
    const parentShowBtnContainer = e.target.parentElement;
    const inputField = parentShowBtnContainer.previousElementSibling;
    let currentStatus = parentShowBtnContainer.classList.contains("active");
    if (!currentStatus) {
        inputField.setAttribute("type", "text");
    } else {
        inputField.setAttribute("type", "password");
    }
    parentShowBtnContainer.classList.toggle("active");
}
...
if (document.querySelectorAll(".tiny-slider-two-wrapper").length > 0) {
    ...
    let tns3dots = document.querySelectorAll(
...
window.onload = function toastAnimation() {
    ...
    for (let i = 0; i < toastLen; i++) {
        x[i].insertAdjacentHTML(
            "beforeend",
            '<span class="toast-autohide-animation"></span>'
        );
